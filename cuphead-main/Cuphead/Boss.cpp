...
void Boss::update() {
    ...
    // Change direction if the boss reaches the edge of the game area
    if (position.x() <= 0) {
        direction = 1;
    ...
    if (health <= 500) { // 50% от начального здоровья 1000
        currentSpeedX *= 1.5; // Увеличьте скорость на 50% (значение 1.5 можно изменить на любое другое)
    }
